...
public class ArticleEntity {
	...
	// 방향성을 고려해서 ARticle에서 user를 참조해야 Article을 조회할떄 user가 동시에 참조됨
	// @JoinColumn(name="?") 선언으로 생성되는 칼럼명을 설정
	@ManyToOne(fetch = FetchType.LAZY)
	@JoinColumn(name="writer")
	private UserEntity user;
	
	//OneToOne을 사용했으나 원래 OneToMany이다.
	@OneToOne(fetch = FetchType.LAZY, mappedBy = "article")
	private FileEntity file;
	
	// 방향성을 고려해서 ARticle에서 comment를 참조해야 Article을 조회할떄 comment가 동시에 참조됨
